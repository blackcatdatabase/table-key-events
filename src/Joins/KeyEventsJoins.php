...
namespace BlackCat\Database\Packages\KeyEvents\Joins;
...
final class KeyEventsJoins {
    ...
    /**
     * FK: key_events -> users
     * LEFT JOIN vw_users AS $as ON $as.id = $alias.actor_id
     * @return array{0:string,1:array<string,mixed>}
     */
    public function joinUsers(string $alias = 't', string $as = 'j1'): array {
        [$alias, $as] = $this->assertAliasPair($alias, $as);
        return [' LEFT JOIN vw_users AS ' . $as . ' ON ' . $as . '.id = ' . $alias . '.actor_id' . ' ', []];
    }
