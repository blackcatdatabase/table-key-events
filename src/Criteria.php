...
final class Criteria extends BaseCriteria
{
    ...
    {
        return [ 'id', 'key_id', 'basename', 'event_type', 'actor_id', 'job_id', 'note', 'meta', 'source', 'created_at' ];
    }

    /** Columns used for full-text LIKE/ILIKE searches. */
    protected function searchable(): array
    {
        return [ 'basename', 'event_type', 'note', 'source' ];
    }

/** Columns allowed in ORDER BY (falls back to filterable() when empty). */
protected function sortable(): array
{
    return [ 'id', 'key_id', 'basename', 'event_type', 'actor_id', 'job_id', 'note', 'source', 'created_at' ];
}
    ...
    protected function joinable(): array
    {
        ...
        return [ 'crypto_keys' => 'j0', 'users' => 'j1' ];
    }
    ...
    public static function fromDb(
        ...
        if (\method_exists(\BlackCat\Database\Packages\KeyEvents\Definitions::class, 'softDeleteColumn')) {
            $soft = \BlackCat\Database\Packages\KeyEvents\Definitions::softDeleteColumn();
