...
use BlackCat\Database\Packages\KeyEvents\Dto\KeyEventDto as Dto;
use BlackCat\Database\Packages\KeyEvents\Mapper\KeyEventDtoMapper as RowMapper;
...
final class KeyEventRepository implements RepoContract, KeysetRepoContract
{
     ...
     */
    protected function def(): string { return \BlackCat\Database\Packages\KeyEvents\Definitions::class; }
    ...
    public function upsertMany(array $rows): int {
        ...
        if (class_exists(\BlackCat\Database\BulkUpsertRepository::class) && [] !== []) {
            $bulk = new \BlackCat\Database\BulkUpsertRepository($this->db, \BlackCat\Database\Packages\KeyEvents\Definitions::table());
    ...
    */
    ...
    /** @return array<string,mixed>|\BlackCat\Database\Packages\KeyEvents\Dto\KeyEventDto|null */
    public function getById(int $id, bool $asDto = false): array|\BlackCat\Database\Packages\KeyEvents\Dto\KeyEventDto|null {
        $row = $this->getByUnique([ 'id' => $id ]);
        if (!$asDto || !$row) return $row;
        return \BlackCat\Database\Packages\KeyEvents\Mapper\KeyEventDtoMapper::fromRow($row);
    }
