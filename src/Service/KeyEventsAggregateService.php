 ...
 */
final class KeyEventsAggregateService
{
use ServiceHelpers;
    ...
    public function __construct(
        private Database $db, private KeyEventRepository $keyEventRepo
    ) {
        // Repository autowiring (generated)
        $this->repositories = [
  'key_events' => $this->keyEventRepo
];
    }
    ...
    protected function withRowLock(callable $locker, int|string|array $id, callable $fetch, string $mode = 'wait'): mixed
    {
        ...
        return $this->txn(function () use ($locker, $id, $fetch, $mode) {
            ...
            if ($row === null) {
                ...
                throw new \BlackCat\Database\Packages\KeyEvents\ModuleException("Record {$idStr} not found for locking.");
            }
